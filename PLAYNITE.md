...
2. **Без доступа к репе** — `scripts/upload_playnite_remote.py` заливает конвертированный JSON в удалённое хранилище (рекомендуется Pantry — getpantry.cloud; npoint закрыл запись для бинов с аккаунтом), сайт сам подтянет новые игры (см. `STREAMER_NO_REPO.md` раздел про `gamesUrl`)
